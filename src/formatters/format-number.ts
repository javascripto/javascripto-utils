...
export function createTruncateNumberFormatter(locale: Intl.LocalesArgument) {
  return (value: number | null = 0, decimals = 2) =>
...
export const formatNumberBR = (number?: number | null, decimals = 2) =>
  isNullOrUndefined(number) ? '' : truncateNumberBR(number, decimals);

export const formatNumberUS = (number?: number | null, decimals = 2) =>
