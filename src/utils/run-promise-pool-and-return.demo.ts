...
function createRandomTimeTask(id: number, maxDelay: number): Task<number> {
  return async (signal?: AbortSignal): Promise<number> => {
    const randomTime = randomInt({ max: maxDelay });
    await wait(randomTime);
    if (signal?.aborted) throw new Error(`Task ${id} aborted before start`);
    console.info(
...
async function main() {
  ...
  const { results: tasksResults, errors } = await runPromisePoolAndReturn({
    tasks,
    concurrencyLimit,
